  ...
  for (let row = 0; row < 8; row++) {
    ...
    for (let col = 0; col < 8; col++) {
      ...
      if (candy && candy.type === currentType) {
        currentMatch.push({ ...candy, row, col });
      } else {
        ...
        currentMatch = candy ? [{ ...candy, row, col }] : [];
...
export const wouldCreateMatch = (board, row1, col1, row2, col2) => {
  ...
  if (tempBoard[row1][col1]) {
    tempBoard[row1][col1] = { ...tempBoard[row1][col1], row: row1, col: col1 };
  }
  if (tempBoard[row2][col2]) {
    tempBoard[row2][col2] = { ...tempBoard[row2][col2], row: row2, col: col2 };
  }
